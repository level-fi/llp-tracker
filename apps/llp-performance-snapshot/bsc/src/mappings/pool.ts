...
export function handlePositionIncreased(ev: IncreasePosition): void {
  ...
  for (let i = 0; i < config.tranches.length; i++) {
    ...
    const feePerShareEntity = new FeePerShare(
      `fee-${tranche.id}-${ev.transaction.hash.toHex()}-${ev.transactionLogIndex}`,
    );
...
export function handlePositionDecreased(ev: DecreasePosition): void {
  ...
  for (let i = 0; i < config.tranches.length; i++) {
    ...
    const feePerShare = share.times(ACC_PRECISION).div(tranche.llpSupply);
    // sig 0: loss, 1: profit
    const pnlPerShare = ev.params.pnl.abs
      .times(ev.params.pnl.sig.equals(ZERO) ? NEGATIVE_ONE : ONE)
      .times(riskFactor_)
      .times(ACC_PRECISION)
    ...
    const pnlPerShareEntity = new PnlPerShare(
      `pnl-${tranche.id}-${ev.transaction.hash.toHex()}-${ev.transactionLogIndex}`,
    );
...
export function handleLiquidityAdded(ev: LiquidityAdded): void {
  ...
  const realizedFeeValue = ev.block.number.ge(config.dao_fee_block_update) ? _calcTotalFee(feeValue) : feeValue;
...
export function handleTokenRiskFactorUpdated(ev: TokenRiskFactorUpdated): void {
  ...
  riskFactor.totalRiskFactor = totalRiskFactor.reverted ? ZERO : totalRiskFactor.value;
